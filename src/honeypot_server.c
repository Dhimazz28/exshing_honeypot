...
#define CONFIG_FILE "/etc/exshing/exshing_honeypot.conf"
...
void start_server() {
    ...
    write_log("Honeypot sedang berjalan");
}
...
void free_request_header(RequestHeader *req) {
    ...
    if (req->request_time) free(req->request_time);
    if (req->encrypted) free(req->encrypted);
}
...
void stop_server(int signal) {
  ...
  {
    close(sock_server);
    write_log("Honeypot telah dihentikan.");
