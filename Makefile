...
ETC_DIR = /etc/exshing
# Direktori Log
LOG_DIR  = /var/log/exshing_honeypot

HTML_SOURCE_DIR = html
HTML_DEST_DIR = /var/www/html/exshing
...
TARGET = $(BIN_DIR)/exshing_honeypot
CONFIG_FILE = config/exshing_honeypot.conf
...
install: all
# Meng-copy-kan exshing_honeypot.conf ke folder /etc/exshing
	@mkdir -p $(ETC_DIR)
	@chmod 0777 $(ETC_DIR)
	@cp $(CONFIG_FILE) $(ETC_DIR)

# Meng-copy-kan contoh HTML ke folder /var/www/html/exshing
...
# Membuat folder log di /var/log/exshing_honeypot
	@mkdir -p $(LOG_DIR)
	@chmod 0777 $(LOG_DIR)
